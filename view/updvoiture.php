...
<div class="jumbotron jumbotron-fluid position-relative overlay-bottom" style="margin-bottom: 90px;">
 <center><a href="listvoiturecond.php"><input type="submit" class="btn btn-secondary px-3 px-lg-5" value="list de mes voitures"></a></center>
    <div class="container text-center my-5 py-5">
        ...
        <form action="updedvoiture.php" method="POST" enctype="multipart/form-data">

        <?php foreach ($list as $voiture) { 
            if(!empty($_POST["matricule"])){
            if ($voiture['matricule'] == $_POST["matricule"]){ ?>

        <br><h2 class="text-center font_trajet  mt-4 mb-4">Modifier la matricule de la voiture:</h2>   
        <input type="text" name="matricule2" value="<?php echo $voiture['matricule']; ?>" class="form-control" placeholder="Matricule" disabled><br>
        <p id="errormatricule" class="error"></p> 

        <h2 class="text-center font_trajet  mt-4 mb-4">Modifier la marque de votre voiture:</h2>   
        <input type="text" name="marque2" value="<?php echo $voiture['marque'];?>" class="form-control" placeholder="Marque"><br>
        <p id="errormarque" class="error"></p> 

        <h2 class="text-center font_trajet  mt-4 mb-4">Modifier la couleur de votre voiture:</h2>   
        <input type="text" name="couleur2" value="<?php echo $voiture['couleur'];?>" class="form-control" placeholder="Couleur"><br>
        <p id="errorcouleur" class="error"></p>

        <h2 class="text-center font_trajet  mt-4 mb-4">Modifier le type:</h2>  
        <input type="text" name="type2" value="<?php echo $voiture['type'];?>" class="form-control" placeholder="Type"><br>
        <p id="errortype" class="error"></p>

        <h2 class="text-center font_trajet  mt-4 mb-4">Modifier l image de la voiture:</h2> 
        <input type="file" id="formFile" value="<?php echo $voiture['photo'];?>" class="form-control" name="file">
        <?php
			}
        }}
			?>
        ...
        <br>
        <input type="submit" value="Modifier voiture" class="btn btn-outline-primary py-3 px-5" name="publier2">
